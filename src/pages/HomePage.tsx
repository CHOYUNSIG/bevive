...
const HomePage: FC = () => {
  return (
    <DimLayout background="black" filter="none" showNavigationBar={true}>
      <div className="flex flex-row h-full">
        <div className="h-full flex-1 flex flex-col justify-center items-center">
          <Carousel images={["/image1.png", "/image2.png", "/image3.png"]} />
        </div>
        <div className="h-full flex-1 flex flex-col justify-center items-center gap-[20px]">
          {[
            {
            ...
            return (
              <div
                key={`button-${i}`}
                className="bg-[#272727] rounded-[20px] px-[14.62px] py-[13.3px] w-[370.62px] flex flex-row gap-[12.52px] items-end"
                onClick={onClick}
              >
                <img className="w-[107.7px]" src={image} alt={`button-${i}`} />
                <div className="">
                  <p className="text-[20px] text-[#D9D9D9] font-['AlteHaasGrotesk'] leading-[40px]">
                    {title}
                  </p>
                  <p className="text-[10px] text-[#D9D9D9] font-light">
                    {description}
                  </p>
                </div>
              </div>
